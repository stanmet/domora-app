...
-- ============================================================
-- 4b. ДОСКА ЗАДАЧ (миграция 20260714161529_task_board_and_offers)
-- ============================================================
...
-- ============================================================
-- 4c. БЕЗОПАСНОСТЬ: включить RLS на всех таблицах
-- ============================================================
-- Supabase поднимает публичный REST-API к таблицам по анонимному ключу. Без RLS
-- любой с этим ключом (а он лежит в браузере) может читать и менять данные.
-- Domora ходит в базу только через Prisma под ролью-владельцем, которая RLS
-- обходит, поэтому включаем RLS без политик: посторонний доступ через авто-API
-- закрыт, а приложение работает как прежде. Идемпотентно, безопасно повторять.
DO $$
DECLARE r RECORD;
BEGIN
