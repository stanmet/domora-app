-- ============================================================================
-- Domora: полная настройка пустой базы Supabase одним скриптом.
-- Вставьте весь файл в Supabase SQL Editor и нажмите Run.
--
-- Что делает скрипт:
--   1. Создаёт все типы (enum), таблицы, индексы и связи
--      (точная копия prisma/migrations/20260712135210_init/migration.sql)
--   2. Регистрирует миграцию в служебной таблице _prisma_migrations,
--      чтобы Prisma считала её уже применённой
--   3. Заполняет seed-данные: 7 категорий услуг и 3 тестовых исполнителя
--      с профилями и активными услугами
--
-- Скрипт рассчитан на пустую базу. Если запустить его второй раз, он просто
-- остановится с ошибкой вида "type already exists" и ничего не испортит:
-- ошибка откатывает весь запуск целиком, данные не задублируются.
-- ============================================================================
...
-- ============================================================================
-- Часть 2. Регистрация миграции для Prisma
-- Prisma хранит список применённых миграций в таблице _prisma_migrations.
-- Записываем туда init-миграцию, чтобы "npx prisma migrate deploy"
-- не пытался применить её второй раз.
-- ============================================================================
...
-- ============================================================================
-- Часть 3. Seed-данные (те же, что в prisma/seed.ts)
-- ============================================================================

-- 3.1 Категории услуг
INSERT INTO "Category" ("id", "slug", "nameEn", "nameRu", "unitDefault", "cancellationTier") VALUES
    ('cat-chef',    'chef',    'Private chef',    'Повар на дом',         'PER_GUEST',   'standard'),
    ('cat-clean',   'clean',   'Cleaning',        'Уборка',               'PER_M2',      'standard'),
    ('cat-handy',   'handy',   'Handyman',        'Мастер на час',        'PER_HOUR',    'standard'),
    ('cat-massage', 'massage', 'Massage',         'Массаж',               'PER_SESSION', 'standard'),
    ('cat-beauty',  'beauty',  'Beauty at home',  'Красота на дому',      'PER_SESSION', 'standard'),
    ('cat-events',  'events',  'Events & parties','Праздники и события',  'PER_EVENT',   'event'),
    ('cat-other',   'other',   'Other',           'Другое',               'FIXED_QUOTE', 'standard')
ON CONFLICT ("slug") DO UPDATE SET
    "nameEn" = EXCLUDED."nameEn",
    "nameRu" = EXCLUDED."nameRu",
    "unitDefault" = EXCLUDED."unitDefault";

-- 3.2 Тестовые исполнители: пользователи
INSERT INTO "User" ("id", "email", "name", "city", "roles") VALUES
    ('seed-user-chef',  'chef@test.domora.ie',  'Aoife Byrne',    'Dublin', ARRAY['PROVIDER']::"Role"[]),
    ('seed-user-clean', 'clean@test.domora.ie', 'Marta Kowalska', 'Cork',   ARRAY['PROVIDER']::"Role"[]),
    ('seed-user-handy', 'handy@test.domora.ie', 'Sean Murphy',    'Galway', ARRAY['PROVIDER']::"Role"[])
ON CONFLICT ("email") DO NOTHING;

-- 3.3 Профили исполнителей
INSERT INTO "ProviderProfile" ("userId", "displayName", "customProfession", "bio", "city", "status") VALUES
    ('seed-user-chef',  'Aoife Byrne',    'Повар на дом',  'Домашние ужины и meal prep на неделю. 8 лет опыта в ресторанах Дублина.', 'Dublin', 'ACTIVE'),
    ('seed-user-clean', 'Marta Kowalska', 'Клинер',        'Генеральная и регулярная уборка квартир и офисов.',                       'Cork',   'ACTIVE'),
    ('seed-user-handy', 'Sean Murphy',    'Мастер на час', 'Мелкий ремонт, сборка мебели, установка бытовой техники.',                'Galway', 'ACTIVE')
ON CONFLICT ("userId") DO NOTHING;

-- 3.4 Услуги (плашки) исполнителей
INSERT INTO "Listing" ("id", "providerId", "categoryId", "professionLabel", "title", "priceCents", "unit", "photos", "status") VALUES
    ('seed-listing-chef',  'seed-user-chef',  'cat-chef',  'Повар на дом',  'Ужин на дому для компании до 8 человек', 4500, 'PER_GUEST', ARRAY[]::TEXT[], 'ACTIVE'),
    ('seed-listing-clean', 'seed-user-clean', 'cat-clean', 'Клинер',        'Генеральная уборка квартиры',            250,  'PER_M2',    ARRAY[]::TEXT[], 'ACTIVE'),
    ('seed-listing-handy', 'seed-user-handy', 'cat-handy', 'Мастер на час', 'Мелкий ремонт и сборка мебели',          3500, 'PER_HOUR',  ARRAY[]::TEXT[], 'ACTIVE')
ON CONFLICT ("id") DO NOTHING;

-- Готово. Проверка: следующие запросы должны вернуть 7 и 3.
