...
## Шаг 5. Stripe (оплата) - в первой версии НЕ НУЖЕН

> **Для запуска V1 этот шаг можно пропустить целиком.** Первая версия Domora
> работает без оплат и комиссий: заказчик и исполнитель договариваются напрямую,
> деньги через сайт не проходят. Платёжный код в проекте сохранён, но выключен -
> без ключей Stripe разделы оплаты просто недоступны, на работу сайта это не
> влияет. Вернитесь к этому шагу позже, когда решите подключить платежи.
>
> Ключи Stripe (`STRIPE_SECRET_KEY`, `NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY` и др.)
> в Vercel добавлять не нужно.

<details>
<summary>Инструкция на будущее (когда захотите включить оплату)</summary>
...
## Шаг 6. Вход через Google и Apple - в первой версии НЕ ИСПОЛЬЗУЕТСЯ

> **Для запуска V1 этот шаг тоже пропускаем.** В первой версии вход только по
> email и паролю (с подтверждением email и восстановлением пароля по почте).
> Кнопок Google/Apple на сайте нет. Инструкция ниже - на будущее, если решите
> добавить вход через соцсети.

<details>
<summary>Инструкция на будущее (вход через Google/Apple)</summary>
...
> **Важно для входа по email.** Письмо-подтверждение при регистрации отправляет
> сам Supabase, а не Resend. На бесплатном тарифе Supabase лимит - всего
> несколько писем в час, этого не хватит для реального потока регистраций.
> Перед запуском в Supabase откройте **Authentication → Emails → SMTP Settings**,
> включите **Custom SMTP** и впишите данные любого почтового сервиса (тот же
> Resend подходит: хост `smtp.resend.com`, порт `465`, логин `resend`, пароль -
> ваш `RESEND_API_KEY`). Тогда подтверждения и восстановление пароля будут
> уходить без ограничений.
